...
public class LevelConfigManager : MonoBehaviour
{
    ...
    public void GetFirstActiveLevelConfig()
    {
        StartCoroutine(FetchFirstActiveLevelConfigCoroutine());
    }
    ...
    private IEnumerator FetchFirstActiveLevelConfigCoroutine()
    {
        string endpoint = $"{baseUrl}/levels/first_active";
            ...
            if (www.result == UnityWebRequest.Result.Success)
            {
                ...
                try
                {
                    ...
                    if (levelResponse.success)
                    {
                        ...
                        Debug.Log($"Successfully retrieved First Active Level Configuration!");
                        LoadedLevelData = levelData;
                        LogLevelData(levelData, levelResponse.timestamp);
                    }
                    else
                    {
                        Debug.LogError($"API Success False (First Active): {levelResponse.message}");
                    }
                }
                catch (System.Exception e)
                {
                    Debug.LogError($"Error parsing JSON response (First Active): {e.Message}\nJSON: {jsonResponse}");
                }
            }
            else
            {
                Debug.LogError($"HTTP Error (First Active): {www.error}\nResponse: {www.downloadHandler.text}");
            }
        }
    }
    ...
    private void LogLevelData(LevelData levelData, string responseTimestamp)
    {
        ...
        mensaje += $"\nPatient ID: {(levelData.patient_id.HasValue ? levelData.patient_id.Value.ToString() : "N/A")}";
